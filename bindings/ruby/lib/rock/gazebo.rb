    ...
    module Gazebo
        ...
        def self.download_missing_models(world_path, wait: false, timeout: 15)
            ...
            if wait
                ...
                # 1. Immediate exit: in case the model is already generated
                unless File.exist?(File.join(stable_temp_dir, model_name))
                    info "Model '#{model_name}' was not found. Waiting up to #{timeout}s for Syskit generation..."

                    notifier = INotify::Notifier.new
                    found = false

                    # Watch for directory creations or folders moved into the temp dir
                    ...
                    unless found
                        raise SDF::XML::NoSuchModel.new(model_name), "Timed out waiting #{timeout}s for model '#{model_name}' to be generated by Syskit."
                    end

                    info "Model '#{model_name}' successfully detected! Resuming execution..."
                end

                # Clear XML model cache and retry loading world
                SDF::XML.clear_cache
                retry
            end

            # 2. Fallback to standard models.gazebosim.org download behavior
