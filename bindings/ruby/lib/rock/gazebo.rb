...
module Rock
    module Gazebo
        ...
        # Encapsulated standard download logic from models.gazebosim.org
        def self.fallback_download(model_name, world_path)
        ...
        def self.wait_for_model_generation(model_name, timeout)
            require 'rb-inotify'
            stable_temp_dir = File.join(Dir.tmpdir, "gazebo_erb_models")
            ::FileUtils.mkdir_p(stable_temp_dir)

            # Immediate exit if Syskit has already generated it
            return true if File.exist?(File.join(stable_temp_dir, model_name))

            info "Model '#{model_name}' was not found. Waiting up to #{timeout}s for Syskit generation..."

            notifier = INotify::Notifier.new
            found = false

            # Watch stable temporary folder for compiled submodel directory creation
            notifier.watch(stable_temp_dir, :create, :moved_to) do |event|
                if event.name == model_name
                    found = true
                    notifier.stop
                end
            end

            # Watchdog thread to interrupt the notifier after timeout
            timeout_thread = Thread.new do
                sleep timeout
                notifier.stop unless found
            end

            # Block and wait (kernel epoll loop)
            notifier.run
            timeout_thread.kill

            if found
                info "Model '#{model_name}' successfully detected! Resuming execution..."
                return true
            else
                return false
            end
        ...
        def self.download_missing_models(world_path, wait: false, timeout: 15)
            waited = false
            downloaded_models = []

            begin
                SDF::Root.load(world_path)
            rescue SDF::XML::NoSuchModel, Errno::ENOENT => error
                model_name = nil

                # 1. Extract the model_name based on the type of error we caught
                if error.is_a?(SDF::XML::NoSuchModel)
                    model_name = error.model_name
                elsif error.is_a?(Errno::ENOENT)
                    failed_path = error.message[/ - (.*)$/, 1]

                    # Only extract if it's an SDF file with an ERB template present
                    if failed_path && failed_path.end_with?('.sdf')
                        dir = File.dirname(failed_path)
                        if !Dir.glob(File.join(dir, '*.erb')).empty?
                            model_name = File.basename(dir)
                        end
                    end
                end

                # If ENOENT triggered but it wasn't a templated model, re-raise the error
                raise error unless model_name

                # Prevent infinite loops if standard download fails to resolve a model
                if downloaded_models.include?(model_name)
                    raise SDF::XML::NoSuchModel, "#{world_path} refers to model #{model_name} which was not available in #{model_path.join(":")}, already tried standard download. You may want to update the GAZEBO_MODEL_PATH environment variable, or set SDF::XML.model_path explicitely."
                end

                if wait && !waited
                    waited = true
                    if wait_for_model_generation(model_name, timeout)
                        SDF::XML.clear_cache
                        retry
                    else
                        warn "Model '#{model_name}' was not generated by Syskit within #{timeout}s. Falling back to standard download behavior..."
                    end
                end

                downloaded_models << model_name
                begin
                    fallback_download(model_name, world_path)
